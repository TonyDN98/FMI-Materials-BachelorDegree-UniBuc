...
arbterm(X,AX) :- nonvar(X), X=..L, (L=[H], AX=arb(H,[]); L=[H,K|T], listarblistterm([K|T],ListSubarb), 
                                   AX=arb(H,ListSubarb)).
...
arbbinterm([O|P], AX):-[O|P]=..[H|T], (T=[], AX=arb(H,nil,nil); T=[H1], arbbinterm(H1,AS), AX=arb(H,AS,nil); 
    T=[H1,H2|_], arbbinterm(H1,AS), arbbinterm(H2,AD), AX=arb(H,AS,AD)). 

arbbinterm(X,AX):-nonvar(X), AX=arb(X,nil,nil).

%arbbinterm(X,AX) :- nonvar(X), (L=[R], AX=arb(R,nil,nil); 
