...
% Rezolvarea Exercitiului 1 din Tema 3:

% prima cerinta:

arbbinterm1(X,arb(X,nil,nil)) :- var(X).
arbbinterm1(X,AX) :- nonvar(X), X=..L, 
		     (L=[R], AX=arb(X,nil,nil); 
		      L=[R,S|T], arbbinterm1(S,AS), (T=[], AD=nil; T\=[], AD=arb(T,nil,nil)), AX = arb(R,AS,AD)).

% a doua cerinta:

arbbinterm2(X,arb(X,nil,nil)) :- var(X).
arbbinterm2(X,AX) :- nonvar(X), X=..L, L=[R|U],
		     ((U=[], ! ; X\=[_|_]), AX=arb(X,nil,nil);
		      X=[H|T], arbbinterm2(H,AS), arbbinterm2(T,AD), AX=arb(R,AS,AD)).

% varianta de rezolvare pentru a doua cerinta in care nu presupunem cunoscuta structura listelor ca termeni,
% ci doar ca listele nevide au operatorul dominant binar:

arbbinterm3(X,arb(X,nil,nil)) :- var(X).
arbbinterm3(X,AX) :- nonvar(X), X=..L, L=[R|U],
		     ((U=[], ! ; X\=[_|_]), AX=arb(X,nil,nil);
		      X=[_|_], U=[H,K], arbbinterm3(H,AS), arbbinterm3(K,AD), AX=arb(R,AS,AD)).

/* Incarcati in interpretorul Prologului acest fisier si fisierul arbbin.pl, si interogati:
?- arbbinterm1(f(f(a,h(b),c),g(h(a),f(X,y,Z)),x), A), reprgr(A).
?- arbbinterm2(f(f(a,h(b),c),g(h(a),f(X,y,Z)),x), A), reprgr(A).
?- arbbinterm3(f(f(a,h(b),c),g(h(a),f(X,y,Z)),x), A), reprgr(A).
?- arbbinterm1([f(a,h(b),c),[h(a),f(X,y,Z)],x], A), reprgr(A).
?- arbbinterm2([f(a,h(b),c),[h(a),f(X,y,Z)],x], A), reprgr(A).
?- arbbinterm3([f(a,h(b),c),[h(a),f(X,y,Z)],x], A), reprgr(A).
Observati ca, daca nu punem "cut" (!) dupa U=[] in "arbbinterm2" si "arbbinterm3", atunci, la ultimele doua interogari de mai sus, daca
cerem mai multe solutii cu ";"/"Next", atunci unica solutie e multiplicata, anume data de atatea ori cate satisfaceri are acea disjunctie. */
