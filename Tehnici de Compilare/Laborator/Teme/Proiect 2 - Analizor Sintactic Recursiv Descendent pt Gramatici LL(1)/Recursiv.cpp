...
void procesareCuvant(string cuvant, int poz, int ok)
{
    // pt eroare
    if(ok == 0) { cout << "Eroare!" << endl; return; }

    // pozitia caracterului curent
    int pozitie = poz;

    // dacă am ajuns la finalul cuvântului, deci am golit stiva
    if(stiva.empty()) { cout << "Ok!" << endl; return; }
    ...
    if(esteTerminal(X))
    {
        if(X == a || (X == '$' && a == '&'))
        {
            stiva.pop();
            pozitie++;
            procesareCuvant(cuvant, pozitie, 1);
        }
        else procesareCuvant(cuvant, pozitie, 0);
    }
    else
    {
