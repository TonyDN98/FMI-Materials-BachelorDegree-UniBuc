...
int main()
{
    ...
    fin>>n; //nr de grade din secventa s0
    ...
    for(i=0; i<n; i++)
    {
        ...
        {cout<<"NU Nu"; return 0;}
        S[i]={x, 'A'+i}; //sau make.pair ---> S contine nodurile cu gradul si eticheta corespunzatoare
    }
    sort(S.begin(), S.end()); //sortam nodurile crescator

    //Afisam Nodurile Initiale
    ...
    while(S[n-1].first>0) //cat timp nu exista valori <=0
    {
        ...
        for(i=1; i<=d; i++) //iau urmatoarele d noduri cele mai mari
        {
            ...
            if(S[n-i].first<0) {cout<<"NU!"; return 0;} //daca avem vreun grad negativ
        }
